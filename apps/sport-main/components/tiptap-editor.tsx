...
interface YoutubeUrlPromptProps {
  onConfirm: (data: { url: string; width: number; height: number }) => void
  onCancel: () => void
}
...
// Modal for Image URL
function ImageUrlPrompt({ onConfirm, onCancel }: ImageUrlPromptProps) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center">
      <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-md">
        ...
        <input
          ...
          className="border border-gray-300 rounded-md w-full p-2 mb-4 text-black"
...
function YoutubeUrlPrompt({ onConfirm, onCancel }: YoutubeUrlPromptProps) {
  const [url, setUrl] = useState("")
  ...
  const handleSubmit = () => {
    if (url) {
      onConfirm({
        url,
        width: Math.max(320, Number.parseInt(width.toString(), 10)) || 640,
        height: Math.max(180, Number.parseInt(height.toString(), 10)) || 480,
      })
    }
  }
  ...
  return (
    <>
      ...
      <div className="border-b border-gray-200 p-2 flex flex-wrap gap-2">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" className="flex items-center gap-1 bg-white text-black hover:bg-gray-100">
...
export function BlogNavbar() {
  return (
    <nav className="sticky top-0 left-0 right-0 z-50 bg-[#0d1b2a] w-full">
      <div className="container mx-auto px-6 md:px-8">
        <div className="flex items-center justify-between h-20">
          <Link href="/">
            <div>
              <span className="text-[#778da9] text-2xl font-bold tracking-wider">[SportsTalk]</span>
              <span className="block text-[#778da9] text-2xl font-bold tracking-wider">Game Analysis Hub</span>
            </div>
          </Link>
          <div className="flex items-center gap-4">
            <Link href="/blog">
              <button className="text-[#778da9] hover:text-[#e0e1dd] flex items-center gap-2">
...
export default function Tiptap() {
  ...
  const editor = useEditor({
    ...
    content: `<h1>Welcome to the Sports Editor!</h1><p>Start writing your sports analysis here. Use the toolbar above to format your content, add images, videos, and more.</p>`,
    ...
    try {
      ...
      if (response.ok) {
        ...
        editor?.commands.setContent(
          "<h1>Welcome to the Sports Editor!</h1><p>Start writing your next sports analysis here.</p>",
        )
  ...
  return (
    <div className="min-h-screen bg-[#006d77] p-6">
      <div className="w-full max-w-6xl mx-auto space-y-4">
        <div className="bg-white border border-[#0d1b2a] rounded-lg p-4">
          <Input
            type="text"
            placeholder="Enter your sports analysis title..."
            value={title}
            onChange={(e) => setTitle(e.target.value)}
            className="text-2xl font-bold border-none p-0 focus-visible:ring-0 placeholder:text-gray-400 text-black"
          />
        </div>

        <div className="border border-[#0d1b2a] rounded-lg overflow-hidden shadow-sm bg-white">
          <MenuBar editor={editor} />
          <EditorContent editor={editor} />
        </div>

        <div className="flex items-center justify-between bg-white border border-[#0d1b2a] rounded-lg p-4">
          <div className="flex items-center gap-4">
            {saveStatus && (
              <Alert
                className={`${saveStatus.includes("successfully") ? "border-green-200 bg-green-50" : "border-red-200 bg-red-50"}`}
              >
                <AlertDescription className={saveStatus.includes("successfully") ? "text-green-800" : "text-red-800"}>
          ...
          <Dialog open={isPublishDialogOpen} onOpenChange={setIsPublishDialogOpen}>
            <DialogTrigger asChild>
              <Button className="bg-[#415a77] hover:bg-[#1b263b] text-white">
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Publish Sports Analysis</DialogTitle>
                <DialogDescription>
                  Are you ready to publish this analysis? Make sure you've reviewed the title and content.
                </DialogDescription>
              </DialogHeader>
              <div className="space-y-4">
                <div>
                  <label className="text-sm font-medium">Analysis Title</label>
                  <Input
                    value={title}
                    onChange={(e) => setTitle(e.target.value)}
                ...
                <div className="flex justify-end gap-2">
                  ...
                  <Button
                    ...
                    className="bg-[#415a77] hover:bg-[#1b263b]"
                  >
                    {isSaving ? "Publishing..." : "Publish Analysis"}
                  </Button>
                </div>
              </div>
            </DialogContent>
